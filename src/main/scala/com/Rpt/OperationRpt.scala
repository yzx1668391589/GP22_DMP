...
//            "(case when LENGTH(androidid)>1 then'安卓' when LENGTH(mac)>1  then '苹果' else '其他' end) types," +
...
//            "from logs group by types").createTempView("log")
//
//          sQLContext.sql("select types,origin,valid,ad,joiner,suc," +
  ...
  */
      val value: RDD[String] = df.rdd.map(row => {
        ...
        if (androidid.length > 1) {
          ("Android", reqlist ++ clicklist ++ allist)
        } else if (mac.length > 1) {
          ("IOS", reqlist ++ clicklist ++ allist)
        } else {
