...
//    sQLContext.sql("select devicetype," +
...
//      "from logs group by devicetype").createTempView("log")
//
//    sQLContext.sql("select (case devicetype when 1 then '手机' when 2 then '平板' else '其他' end) type,origin,valid,ad,joiner,suc," +
//      "(joiner/suc) as sucr,shower,cli,(cli/shower) as clir,DSPcos,DSPcon from log").show()
    ...
    val value: RDD[String] = df.rdd.map(row => {
     ...
     if (devicetype == 1) {
      ...
      (types, reqlist ++ clicklist ++ allist)
