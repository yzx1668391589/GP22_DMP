...
//      "from logs group by ispname").createTempView("log")
//
//    sQLContext.sql("select ispname,origin,valid,ad,joiner,suc," +
...
//      */
        val value: RDD[String] = df.rdd.map(row => {
          ...
          (ispname, reqlist ++ clicklist ++ allist)
