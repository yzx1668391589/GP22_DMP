...
import com.utils.{RptUtils, SchemaUtils2}
...
object LocationRpt {
  def main(args: Array[String]): Unit = {
    ...
    sQLContext.sql("select provincename,cityname," +
      ...
      "from logs group by provincename,cityname").createTempView("log")

    sQLContext.sql("select provincename,cityname,origin,valid,ad,joiner,suc," +
      "(joiner/suc) sucr,shower,cli,(cli/shower) as clir,DSPcos,DSPcon from log").show()
...
//      ((pro, city), reqlist ++ clicklist ++ allist)
