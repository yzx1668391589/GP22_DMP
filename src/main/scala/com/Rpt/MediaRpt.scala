...
import com.utils.{DictionaryUtils, RptUtils}
...
//              "from logs group by appname").createTempView("log")
//
//            sQLContext.sql("select appname,origin,valid,ad,joiner,suc," +
...
//      */
    ...
    val lines = sc.textFile(inputPath1)
    ...
    val broadcast: Broadcast[Map[String, String]] = sc.broadcast(line.collect.toMap)
    ...
    val value: RDD[String] = df.rdd.map(row => {
      ...
      if(appname.length >1){
      (appname, reqlist ++ clicklist ++ allist)
      }else if(appid.length > 1 ){
        (broadcast.value.getOrElse(appid,0),reqlist ++ clicklist ++ allist)
      }else {
        ("其他",reqlist ++ clicklist ++ allist)
      }
